...
void removeBefore(List*& L, int val)
{
    ...
    NODE* cur = L->p_head;
    while (cur->p_next && cur->p_next->p_next)
    {
...
bool addPos(List*& L, int data, int pos)
{
    ...
    cur->p_next = newNode;
    if (cur == L->p_tail) L->p_tail = newNode;
...
{
    if (L->p_head == nullptr) return false;
    if (L->p_head->key == val) return addHead(L, data);
...
/* tra ve list ban dau -> error
List* reverseList(List* L)
{
    if (L == nullptr || L->p_head == nullptr) return L;
...
List* reverseList(List* L)
{
    ...
    reversed->p_head = nullptr;
    reversed->p_tail = nullptr;
    ...
    while (cur)
    {
        ...
        newNode->p_next = reversed->p_head;

        reversed->p_head = newNode;
        if (reversed->p_tail == nullptr) // first node added -> tail
            reversed->p_tail = newNode;
...
void removeDuplicate(List*& L)
{
    ...
    {
        NODE* checker = cur;
        while (checker->p_next)
        {
            if (checker->p_next->key == cur->key)
            {
                NODE* temp = checker->p_next;
                checker->p_next = checker->p_next->p_next;
                delete temp;
            }
            else
            {
...
int main()
{
    ...
    int expectedArr18_5[1] = {}; /////////////// khai bao 0pt -> error
